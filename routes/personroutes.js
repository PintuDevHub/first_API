...
router.get('/',async  (req, res)=>{
    try {
        ...
        console.log(error);
        res.status(500).json({error:'Server issu'});
...
// specific data view by person in db
router.get('/:spcefic', async(req, res)=>{
    try {
        ...
        if(spcefic =='water'|| spcefic == 'sugar' || spcefic == 'chef'){
            const response = await Person.find({work: spcefic});
...
// here person data update 
router.put('/:id', async (req, res)=>{
    try {
        const personId = req.params.id; // Extract the id from the URL parameter
        const updatePersonData = req.body; // Update data for the perosn
